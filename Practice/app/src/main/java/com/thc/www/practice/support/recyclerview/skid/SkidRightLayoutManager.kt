...
import com.thc.www.practice.support.recyclerview.vertical_scroll.ItemViewInfo
import java.util.*
 ...
 * @Date  2018/5/30
 */
class SkidRightLayoutManager(itemHeightWidthRatio: Float, scale: Float) : RecyclerView.LayoutManager() {
    ...
    private var mItemHeightWidthRatio: Float = 0F
    private var mScale: Float = 0F
    private var mItemCount: Int = 0
    private var mSkidRightSnapHelper: SkidRightSnapHelper
    ...
    private fun fill(recycler: RecyclerView.Recycler?) {
        ...
        run {
            ...
            while (i >= 0) {
                val maxOffset = (getHorizontalSpace() - mItemViewWidth) / 2 * Math.pow(mScale.toDouble(), j.toDouble())
    ...
    private fun convert2AdapterPosition(layoutPosition: Int): Int {
        return mItemCount - 1 - layoutPosition
    }

    private fun convert2LayoutPosition(adapterPostion: Int): Int {
