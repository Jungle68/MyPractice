...
public class EditTextAddWrapper<T extends EditText> implements TextWatcher {
    private final T editText;                    // 实际EditText
     ...
     * @return
     */
    private boolean needAdd(String text) {
        if (type == 0) {
            ...
            return (add.length() + perLen) * index + start == len && len < text.length();
        }
    }
     ...
     * @return
     */
    private boolean needDel(String text) {
        if (type == 0) {
            ...
            return index > 0 && (add.length() + perLen) * index - perLen + start == len - 1 && len > text.length();
        }
    }

    private void dealMaxLength() {
